...
|-----------|----------|----------------------|-----------|----------|
| Tôi (Nam) | ArticleChunker (1 Điều/chunk) | 10/10 | Giữ trọn vẹn ý nghĩa của một Điều luật, không tách rời các Khoản liên quan và đạt 5/5 query relevant trong top-3. | Một số Điều dài làm chunk khá lớn, có thể khiến câu trả lời chứa thêm chi tiết ngoài trọng tâm. |
| Quân | Article-level + Hybrid BM25/Vector | 10/10 | BM25 bắt đúng thuật ngữ/tiêu đề Điều, vector search bổ sung semantic match và đạt 5/5 top-1 trong benchmark cá nhân. | Cần metadata và heading sạch; cách implement phức tạp hơn ArticleChunker thuần. |
| Đạt | RecursiveChunker (chunk_size=400) | 10/10 (Local top-3) | Chunk nhỏ hơn, linh hoạt hơn với câu hỏi ngắn và vẫn bảo toàn ranh giới đoạn/câu tốt. | Có thể tách một Điều thành nhiều mảnh, nên query pháp lý cần đủ ngữ cảnh có thể thiếu vài khoản liên quan. |
| Khoa | ArticleChunker + 8 file + chapter filter | 8/10 (Local top-1) | Tách dữ liệu theo chương và filter `chapter` giúp giảm nhiễu mạnh, đặc biệt với câu hỏi có phạm vi chương rõ. | Cần metadata/filter chính xác; nếu filter hoặc chương chưa khớp, kết quả có thể bỏ sót Điều đúng. |

**Strategy nào tốt nhất cho domain này? Tại sao?**
> Dựa trên kết quả thực tế của nhóm, **ArticleChunker** là strategy phù hợp nhất cho domain "Luật Trí tuệ nhân tạo" vì văn bản luật được tổ chức theo Điều, Khoản rất chặt chẽ. Phiên bản mạnh nhất là ArticleChunker kết hợp thêm metadata/filter và hybrid BM25/vector như Quân đã thử, còn ArticleChunker thuần vẫn có ưu điểm là đơn giản, dễ kiểm soát và giữ đầy đủ ngữ cảnh của từng Điều.
