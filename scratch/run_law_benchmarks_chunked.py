...
# Load and chunk the 8 chapters of Luật AI
docs = []
...
for i in range(1, 9):
    ...
    # Chia nhỏ chương này thành nhiều chunk
    ...
    for j, chunk_content in enumerate(chunks):
        ...
        docs.append(Document(id=f"luat_ai_chuong{i}_chunk_{j}", content=chunk_content, metadata=meta))

# Initialize store with MockEmbedder
store = EmbeddingStore(collection_name="law_store_chunked", embedding_fn=_mock_embed)
store.add_documents(docs)

# Initialize agent with a mock LLM that returns a clean snippet of the retrieved context
def test_llm(prompt: str) -> str:
    # We retrieve Context 1 from the prompt
    if "Context 1:" in prompt:
        context_segment = prompt.split("Context 1:")[1].split("\n")[0][:120]
        return f"[Mock LLM] Trả lời: {context_segment.strip()}..."
...
print("Index size (chunks):", store.get_collection_size())
print("\n--- CHUNKED BENCHMARK RESULTS ---")
